...
- Safer path handling for files with spaces (texture crops, model lists, atlas inputs).
- Automatic JSON normalization to strip UTF-8 BOM before `jq` parsing.
- Early abort with a clear error if no valid models remain (prevents silent hangs).
- Consolidation step fixed so renamed duplicates stay inside `target/rp/...` instead of spilling into repo root.
- Cleanup typo fixed for `target/rp/attachables` consolidation.
