...
# strip UTF-8 BOMs from JSON files so jq can parse exported model files reliably
normalize_json_encodings () {
  local target_dir="${1}"
  if [[ -d "${target_dir}" ]]; then
    status_message process "Normalizing JSON encodings in ${target_dir}"
    while IFS= read -r -d '' json_file; do
      perl -0pi -e 's/^\xEF\xBB\xBF//' "${json_file}"
    done < <(find "${target_dir}" -type f -name '*.json' -print0)
    status_message completion "JSON encodings normalized in ${target_dir}"
  fi
}
...
normalize_json_encodings "./assets"
...
while IFS= read -r -d '' json_file; do
  json_dir+=("${json_file}")
done < <(find ./assets/**/models -type f -name '*.json' -print0)
...
mapfile -t model_array < <(jq -r '[.[].path] | unique | .[]' config.json)

# drop entries that point to invalid JSON model files so later jq steps do not empty config and hang.
valid_model_array=()
invalid_model_array=()
for model_file in "${model_array[@]}"; do
  if jq -e . "${model_file}" > /dev/null 2>&1; then
    valid_model_array+=("${model_file}")
  else
    invalid_model_array+=("${model_file}")
  fi
done

if [[ ${#invalid_model_array[@]} -gt 0 ]]
then
  printf '%s\n' "${invalid_model_array[@]}" | sponge scratch_files/invalid_model_json.txt
  status_message critical "Removing config entries with invalid model JSON definitions"
  jq '

def invalid_file($input):
($ARGS.positional | index($input) // null);

map_values(if invalid_file(.path) == null then . else empty end)

' config.json --args "${invalid_model_array[@]}" | sponge config.json
  mapfile -t model_array < <(jq -r '[.[].path] | unique | .[]' config.json)
fi

if [[ ${#model_array[@]} -eq 0 ]]
then
  status_message error "No valid model files remained in the predicate config. Aborting to avoid hanging during atlas generation."
  exit 1
fi
...
done < <(find ./assets/**/textures -type f -name "*.mcmeta" -print0)
...
mapfile -t model_list < <(jq -r '.[] | select(.generated == false) | .path' config.json)

if [[ ${#model_list[@]} -eq 0 ]]
then
  status_message error "No valid models remained after preprocessing. Aborting to avoid hanging during atlas generation."
  exit 1
fi
...
while IFS= read -r -d '' texture_file; do
  all_texture_files+=("${texture_file}")
done < <(find ./assets/**/textures -type f -name '*.png' -print0)
jq -n '$ARGS.positional' --args "${all_texture_files[@]}" | sponge scratch_files/all_textures.temp
...
do
  generate_atlas () {
    ...
    local texture_list=()
    mapfile -t texture_list < <(jq -s --arg index "${1}" -r '(.[1][($index | tonumber)] - .[0] | length > 0) as $fallback_needed | ((.[1][($index | tonumber)] - (.[1][($index | tonumber)] - .[0])) + (if $fallback_needed then ["./assets/minecraft/textures/0.png"] else [] end)) | .[]' scratch_files/all_textures.temp scratch_files/union_atlas.temp)
...
then
    ...
    function consolidate_files () {
        ...
        list=$(find "${target_dir}" -mindepth 2 -type f -print)

        ## Rename any remaining collisions and keep them inside the target pack
        while [[ -n "${list}" ]] ; do
           find "${target_dir}" -mindepth 2 -type f -print0 | while IFS= read -r -d '' file; do
               local current_file
               current_file=$(basename "$file")
               mv -n "$file" "${target_dir}/${nr}${current_file}"
           done
           nr=$((nr+1))
           list=$(find "${target_dir}" -mindepth 2 -type f -print)
        done
